...
const AuthResetPassword = () => {
  ...
  useEffect(() => {
    ...
    if (storedToken) {
      setFormData((prevData) => ({ ...prevData, token: storedToken }));
    } else {
      navigate("/password-reset"); // Redirect if email is not found
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8080/user/password/reset/submit", formData, {
        headers: {
