...
const OTP = () => {
  ...
  const { successMessage } = location.state || {}; // Get successMessage and token from state

  const [otp, setOtp] = useState(""); // State to handle OTP input
  const [email, setEmail] = useState(""); // State to store email from localStorage
  ...
  useEffect(() => {
    ...
    if (storedEmail) {
      setEmail(storedEmail); // Set email state if it exists in localStorage
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    // Create form data with OTP, token, and email from localStorage
    const formData = {
      otp: otp, // The entered OTP
      email: email, // The email from localStorage
    };

    try {
      // Make POST request to /verify/signup/otp/submit
      const response = await axios.post(
        "http://localhost:8080/user/verify/forget/password/otp/submit",
        formData,
        {
          headers: {
      ...
      if (response.data) {
        ...
        // Pass success message when navigating to /reset-password
